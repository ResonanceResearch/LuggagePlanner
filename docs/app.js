...
function renderItemLibrary() {
  ...
  el.itemLibrary.querySelectorAll(".library-item").forEach((card) => {
    card.addEventListener("dragstart", (event) => {
      ...
      event.dataTransfer.setData("text/plain", JSON.stringify({ kind: "library-item", itemId: card.dataset.itemId }));
    });
...
function bindBagInteractions() {
  ...
  el.bagGroups.querySelectorAll(".placement-item").forEach((card) => {
    card.addEventListener("dragstart", (event) => {
      ...
      event.dataTransfer.setData("text/plain", JSON.stringify({ kind: "placement", placementId: card.dataset.placementId }));
    });
    card.addEventListener("dragend", () => card.classList.remove("dragging"));
  });
